...
//               2. Roll the dice and the first number 1-6 decides which skater moves. If 6 is rolled then ALL skaters move.
...
struct ContentView: View {
    ...
    // State variables to show the winners in a new screen
                        ...
                        HStack {
                            ...
                            VStack(alignment: .leading) {
                                Button(
                                 "Roll",
                                 action: {
                                   //the button rolls the dice to see which skater moves and how far they move
                                   ...
                                   // depending on the dice values this code moves the skater
                                   let skaterPosChange = CGFloat(skaterStrides * 10)
                                     ...
                                     // After every move this code checks to see if any skater crossed the finish line
...
// This function is called twice by the Roll button action. Once for the skater and once for the distance
...
func checkForWinner(skater1Distance: CGFloat, skater2Distance: CGFloat, skater3Distance: CGFloat, skater4Distance: CGFloat, skater5Distance: CGFloat, strWinners: inout String) -> Bool {
    // This function takes in all 5 skater positions and checks to see which skaters crossed the finish line
    
    // skaters start at a -550 Y position so the finishline is at 0
    let finishLineYPos: CGFloat = 0
    var nbrWinners: Int = 0
    
    // check each skater to see if they crossed the finishline
    // I use nbrWinners to increment how many skaters cross the finishline because if it's more than 1 we have a tie
    ...
    // The code determines whether or not we have a winner and if we have a winner we determine if it's a single skater or multiple skaters
  
    // if it's only 1 winner
    if nbrWinners == 1 {
        strWinners = "Skater " + strWinners + " won the race"
        return true
    }
    // if there is a tie with more than 1 winner
